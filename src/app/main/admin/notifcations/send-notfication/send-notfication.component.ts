...
@Component({
  selector: 'app-send-notfication',
  templateUrl: './send-notfication.component.html',
  styleUrls: ['./send-notfication.component.scss'],
  encapsulation:ViewEncapsulation.None
})


export class SendNotficationComponent implements OnInit {
  sendNotifactionsOptoinsModal: IModal;
  @ViewChild("SendNotficationsModal") SendNotficationsModal:
    | ElementRef
    | undefined;

    @ViewChild("SendBrodcatsNotficationsModal") SendBrodcatsNotficationsModal:
      | ElementRef
      | undefined;
  ...
  BrodcatsnotifactionsForm: FormGroup;
  ...
  get BrodcatsnotifactionsFormControls() {
    return this.BrodcatsnotifactionsForm.controls;
  }
  ...
  InitNotifactionsForm() {
    ...
    this.BrodcatsnotifactionsForm = this.fb.group({
      userIds: [[]],
      title: ["", [Validators.required]],
      body: [""],
      image: [""],
            ...
            if (event.type == HttpEventType.UploadProgress) {
              ...
              if (event.body) {
                if (event.body.data) {
                 ...
                 if(type=='bordcats'){
                  this.BrodcatsnotifactionsForm.controls.image.setValue(
                    event.body.data
                  );
                 }else{
  ...
  closeModalBrodcat() {
    this._customModalService.dissmissMoadal(this.SendBrodcatsNotficationsModal);
  }
  
  users=[];
  bordcatNotfiactions($event){
    if ($event.length>0) {
        this.users=$event;
    }else{
      this.users=[];
  ...
  SendNotficationToAllUsers(){
    console.log('users',this.users);
    
   if(this.users.length>=2){
    this._customModalService.openModal(this.SendBrodcatsNotficationsModal, "md");

   }else{
    Swal.fire({
      title: "لابد من اختيار اكثر من مستخدم لارسال الاشعار لهم ",
      icon: "warning",
  ...
  SendBrodcatsNotfication() {
    this.submitted = true;

    console.log("form value: ", this.BrodcatsnotifactionsForm.value);

    if (this.BrodcatsnotifactionsForm.invalid) {
      return;
    }
  ...
  this.BrodcatsnotifactionsForm.get('userIds').patchValue(arrUsers);
    this._httpService
      .post("/Notifications/PushNotificationForUsers",this.BrodcatsnotifactionsForm.value, false)
      .subscribe(
        (res: any) => {
          console.log(res);
          if (res) {
            ...
            this.resetBrodcatsnotifactionsForm();
            this.loading = false;
          }
        },
        (err) => {
  ...
  resetBrodcatsnotifactionsForm() {
    this.submitted = false;
    this.BrodcatsnotifactionsForm.reset();
    this.closeModalBrodcat();
  }
  deleteUser(id: any) {
