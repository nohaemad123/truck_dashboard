...
export class SettingsService {
  ...
  public setAppDirectionFromLocalStorage() {
    let AppDirection = localStorage.getItem("AppDirection")?localStorage.getItem("AppDirection"):'rtl';
    if (AppDirection === 'rtl' || AppDirection === 'ltr') {
      this.setAppDirection(AppDirection)
    }
  }
  private mobileLayout: BehaviorSubject<boolean>= new BehaviorSubject(false);
  public setAppDirection(AppDirection: 'ltr' | 'rtl') {
    try {
      let renderer = this.rendererFactory.createRenderer(null, null);
      if (AppDirection) {
        localStorage.setItem("AppDirection", AppDirection);
        if (AppDirection === 'rtl') {
  ...
  public getIsMobileView() {
    return this.mobileLayout;
  }
  
  public setAppMobileLayout() {

    try {
      let renderer = this.rendererFactory.createRenderer(null, null);
      if (this.checkMobileView()) {
        this.mobileLayout.next(true);
        renderer.setAttribute(document.body, 'mobile-layout', 'true');
      } else {
        this.mobileLayout.next(false);
        renderer.setAttribute(document.body, 'mobile-layout', 'false');
      }
